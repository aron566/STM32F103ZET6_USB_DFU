 ...
 * @param block_mode IT mode
 *
 * @return >= 0: successful write data size
 *           -1: error
 */
int fal_partition_write(const struct fal_partition *part, uint32_t addr, const uint8_t *buf, size_t size, bool block_mode)
{
    int ret = 0;
    const struct fal_flash_dev *flash_dev = NULL;
    ...
    ret = flash_dev->ops.write(part->offset + addr, buf, size, block_mode);
    if (ret < 0)
    {
 ...
 *           -1: error
 */
int fal_partition_erase(const struct fal_partition *part, uint32_t addr, size_t size, bool block_mode)
{
    int ret = 0;
    const struct fal_flash_dev *flash_dev = NULL;
    ...
    ret = flash_dev->ops.erase(part->offset + addr, size, block_mode);
    if (ret < 0)
    {
 ...
 *           -1: error
 */
int fal_partition_erase_all(const struct fal_partition *part, bool block_mode)
{
    return fal_partition_erase(part, 0, part->len, block_mode);
}
